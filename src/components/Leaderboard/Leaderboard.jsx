...
class Leaderboard extends React.Component {
    ...
    searchDetails(event){
        ...
        // this.searchReg=event.target.value;
        console.log("search reg",this.state.searchReg);
    }
        ...
        return(
            <Background>
                ...
                <div className={classes.searchTopContainer}>
                    <input type="text" placeholder="Enter Regisration Number" name="search" className={classes.searchTop} autoComplete="off" onChange={this.searchDetails}/>
                    <img src={search} className={classes.btnTop} alt="search" onClick={this.getUser}/>
                </div> */}
            ...
            <form>
                <div className={classes.formContainer} onChange={this.getSelectedVal}>
                    ...
                    <div className={classes.labelContainer}>
                        <svg width="25" height="25" viewBox="0 0 55 43" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div className={classes.labelContainer}>
                        <svg width="25" height="25" viewBox="0 0 45 56" xmlns="http://www.w3.org/2000/svg">
...
class Display extends React.Component {
    ...
    render(){
        const colors = ["#475897", "#7A12CB","#AA1047"];
                    ...
                    {this.props.details.map((deet,index) => {
                        return (
                            <div className={classes.textContainer} >
                                ...
                                {this.props.selected!=='management' ? 
                                    <div className={classes.mainDetails}>
                                        <div className={classes.imgContainer}><img id='userpic' src={userPic} alt='User pic' style={mystyle}/><h4>{deet.regno} - {deet.name}</h4></div>
                                        <div className={classes.box}>
                                            <h4>{deet.techscore>=0 ? deet.techscore : deet.designscore} Points</h4>
                                            <button className={classes.expand} onClick={this.setShowInfo}>{this.state.showInfo ? "–" : "+" }</button>
                                        </div>
                                    </div> :
                                <div className={classes.mainDetails}>
                                    <div className={classes.imgContainer}><img id='userpic' src={userPic} alt='User pic' style={mystyle}/><h4>{deet.name}</h4></div>
                                    <div className={classes.box}>
                                        <h4>{deet.regno}</h4>
                                        <button className={classes.expand} onClick={this.setShowInfo}>{this.state.showInfo ? "–" : "+" }</button>
                                    </div>
                                    ...
                                    <div className={classes.selectionContainer}>
                                        <input type="radio" value="accept" name="selection2" id="accept" className={classes.input1} onChange={this.rejectUser}></input>
                                        <label htmlFor="accept" className={classes.selection}><p className={classes.select}>Accept</p></label>
                                        <input type="radio" value="reject" name="selection2" id="reject" className={classes.input2}></input>
                                        <label htmlFor="reject" className={classes.selection}><p className={classes.select}>Reject</p></label>
                                    </div>
                                    <p>Phone: {deet.phone}</p>
                                    <p>Email: {deet.email}</p>
                                </div>: null}
                                </div>
                            </div>
                        )
